...
def antigravity(
    ...
    async def execute(state: AgentState) -> AgentState:
        ...
        async with sandbox_agent_bridge(
            ...
            # Bridged MCP endpoints must be live BEFORE launch: the runner
            # reads its MCP config at startup and the bridge proxy starts
            # asynchronously, so an early launch yields an agent with no
            # bridged tools and NO error, whose output is then scored as a
            # valid trajectory. Raises if unreachable. #101 integration:
            # antigravity is a sixth self-launching bridged agent and must
            # gate like codex_cli/claude_code.
