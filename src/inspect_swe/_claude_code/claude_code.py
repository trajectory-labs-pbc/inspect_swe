...
def claude_code(
    ...
    Args:
        ...
        allowlist_mcp_tools: Whether to add bridged/static MCP tools to
            `--allowed-tools` (default `True`). Set `False` only under a
            permission mode whose first-party classifier should review every
            MCP call (e.g. `permission_mode="auto"`): an allow rule resolves
            before the classifier and would bypass it, so suppressing the
            allow-list forces each bridged call to be adjudicated. The servers
            are still registered via `--mcp-config`, so the tools remain
            invocable. Leave `True` for `--dangerously-skip-permissions` and
            `acceptEdits`, where the allow-list is required for MCP tools to
            work at all.
