...
def codex_cli(
    ...
    **deprecated_args: Unpack[CodexDeprecatedArgs],
    ...
    Args:
        ...
        mcp_ready_timeout: Seconds to wait for bridged MCP endpoints to serve tools
            before the agent launch errors.
    ...
    async def execute(state: AgentState) -> AgentState:
        ...
        async with (
            ...
            # Bridged servers are required and receive the configurable Codex startup
            # deadline. Their tools bypass the headless approval gate; static servers
            # retain Codex's default unless the caller explicitly opts them in.
            bridged_server_names = {server.name for server in bridge.mcp_server_configs}
            ...
            for mcp_server in all_mcp_servers:
                ...
                if mcp_server.name in bridged_server_names or approve_static_mcp_tools:
                    server_toml = codex_mcp_server_toml(
                        server_toml, effective_approval_policy
                    )
