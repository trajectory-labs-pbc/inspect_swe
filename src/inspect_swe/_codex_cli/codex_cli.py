    ...
    **deprecated_args: Unpack[CodexDeprecatedArgs],
    ...
    async def execute(state: AgentState) -> AgentState:
        ...
        async with (
            ...
            # Bridged tools are marked required so a failed MCP startup fails
            # the launch loudly instead of running the turn without tools (see
            # codex_mcp_server_config); their tool-approval gate is always
            # skipped via codex_mcp_server_toml since bridged servers are
            # supplied by the evaluation author.
            bridged_server_names = {s.name for s in bridge.mcp_server_configs}
            for mcp_server in bridge.mcp_server_configs:
                bridged_toml = codex_mcp_server_config(
                    mcp_server, bridged_server_names, mcp_startup_timeout
                )
                bridged_toml = codex_mcp_server_toml(
                    bridged_toml, effective_approval_policy
                )
