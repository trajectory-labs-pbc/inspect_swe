...
def codex_mcp_server_toml(
    ...
    `required=True` additionally marks the server required: codex >= 0.119.0
    then blocks session init on its initialize+tools/list and `codex exec`
    exits non-zero if it fails to come up, closing the client-connect half of
    the first-turn race (`wait_for_mcp_endpoints` covers the endpoint half;
    Claude Code's equivalent is `BLOCKING_MCP_ENV`). Older codex versions
    ignore the key. Only bridged servers pass it -- a static caller-provided
    server's availability is the caller's contract, mirroring the
    readiness-gate scoping.
    """
    mcp_server_toml = dict(mcp_server_dump)
    if approval_policy == "never":
        mcp_server_toml["default_tools_approval_mode"] = "approve"
    if required:
        mcp_server_toml["required"] = True
